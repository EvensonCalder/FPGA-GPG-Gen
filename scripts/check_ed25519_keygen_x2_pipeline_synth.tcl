set clk_period [expr {[info exists ::env(CLK_PERIOD)] ? $::env(CLK_PERIOD) : "20.000"}]
set suffix [expr {[info exists ::env(PROJ_SUFFIX)] ? $::env(PROJ_SUFFIX) : [string map {. p} $clk_period]}]
set proj_dir [file normalize "build/vivado_ed25519_keygen_x2_pipeline_synth_${suffix}"]
set ref_dir  [file normalize "saif_ed25519_ref"]

file mkdir $proj_dir
create_project ed25519_keygen_x2_pipeline_synth_${suffix} $proj_dir -part xc7k160tffg676-2 -force
...
foreach f {
    ...
    rtl/ed25519_keygen_x2_pipeline.sv
} {
    add_files [file normalize $f]
    set_property file_type SystemVerilog [get_files [file normalize $f]]
}

set_property top ed25519_keygen_x2_pipeline [current_fileset]
synth_design -top ed25519_keygen_x2_pipeline -part xc7k160tffg676-2 -mode out_of_context
