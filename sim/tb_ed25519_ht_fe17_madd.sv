...
module tb_ed25519_ht_fe17_madd;
    ...
    task automatic run_vector(input int idx);
        ...
        begin
            ...
            if (r_X !== fe17_t'(vec[idx][7]))
                $fatal(1, "X mismatch vector=%0d got=%064x expected=%064x", idx, r_X, vec[idx][7]);
            if (r_Y !== fe17_t'(vec[idx][8]))
                $fatal(1, "Y mismatch vector=%0d got=%064x expected=%064x", idx, r_Y, vec[idx][8]);
            if (r_Z !== fe17_t'(vec[idx][9]))
                $fatal(1, "Z mismatch vector=%0d got=%064x expected=%064x", idx, r_Z, vec[idx][9]);
            if (r_T !== fe17_t'(vec[idx][10]))
                $fatal(1, "T mismatch vector=%0d got=%064x expected=%064x", idx, r_T, vec[idx][10]);

            $display("PASS madd vector=%0d cycles=%0d", idx, cycles);
    ...
    initial begin
        ...
        $display("PASS tb_ed25519_ht_fe17_madd vectors=%0d", VECTOR_COUNT);
        $finish;
    end
endmodule
