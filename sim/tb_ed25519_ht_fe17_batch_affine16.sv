...
module tb_ed25519_ht_fe17_batch_affine16;
    logic clk = 1'b0;
    logic rst_n = 1'b0;
    always #2 clk = ~clk;
    ...
    logic start;
    logic done;
    logic [255:0] seed_in [0:15];
    fe17_t x [0:15], y [0:15], z [0:15];
    fe17_t t_unused [0:15];
    logic [255:0] expected_public [0:15];
    logic [255:0] seed_out [0:15];
    logic [255:0] public_key [0:15];

    ed25519_ht_fe17_batch_affine16 dut (
    ...
    task automatic load_vectors;
        ...
        begin
            if (!$value$plusargs("VECTOR_FILE=%s", vector_file))
                vector_file = "sim/ht_batch_affine16_vectors.mem";
    ...
    initial begin
        ...
        begin
            int cycles = 0;
            while (!done && cycles < 60000) begin @(posedge clk); cycles++; end
            if (!done) $fatal(1, "batch affine16 timeout");
            #1;
            for (int i = 0; i < 16; i++) begin
            ...
            $display("PASS tb_ed25519_ht_fe17_batch_affine16 cycles=%0d", cycles);
        end
        $finish;
    end
endmodule
