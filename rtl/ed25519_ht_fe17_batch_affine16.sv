...
module ed25519_ht_fe17_batch_affine16 (
    input  logic         clk,
    input  logic         rst_n,
    input  logic         start,
    input  logic [255:0] seed_in [0:15],
    input  fe17_t        x_in [0:15],
    input  fe17_t        y_in [0:15],
    input  fe17_t        z_in [0:15],
    output logic         done,
    output logic [255:0] seed_out [0:15],
    output logic [255:0] public_key [0:15]
);
    ...
    logic [3:0] idx;
    logic [3:0] aff_idx;
    fe17_t x_q [0:15], y_q [0:15], z_q [0:15];
    fe17_t prefix [0:15];
    fe17_t inv_z [0:15];
    fe17_t inv_acc;
    fe17_t inv_all;
    ...
    ed25519_ht_fe17_invert_pow u_inv (.clk(clk), .rst_n(rst_n), .start(inv_start), .z(prefix[15]), .busy(), .done(inv_done), .out(inv_all));
        ...
        if (!rst_n) begin
            ...
            unique case (state)
                ...
                ST_PREFIX_WAIT: if (mul0_done) begin
                    prefix[idx] <= mul0_y;
                    if (idx == 4'd15) state <= ST_INV_START;
                    else begin idx <= idx + 4'd1; state <= ST_PREFIX_START; end
                end
                ST_INV_START: state <= ST_INV_WAIT;
                ST_INV_WAIT: if (inv_done) begin inv_acc <= inv_all; idx <= 4'd15; state <= ST_BACK_START; end
                ST_BACK_START: state <= ST_BACK_WAIT;
                ST_BACK_WAIT: if (mul0_done && mul1_done) begin
                    inv_z[idx] <= mul0_y;
                    inv_acc <= mul1_y;
                    if (idx == 4'd1) state <= ST_INVZ0_ASSIGN;
                    else begin idx <= idx - 4'd1; state <= ST_BACK_START; end
                end
                ST_INVZ0_ASSIGN: begin inv_z[0] <= inv_acc; aff_idx <= 4'd0; state <= ST_AFF_START; end
                ST_AFF_START: state <= ST_AFF_WAIT;
                ST_AFF_WAIT: if (mul0_done && mul1_done) begin
                    public_key[aff_idx] <= encoded_mul_public;
                    if (aff_idx == 4'd15) state <= ST_DONE;
                    else begin aff_idx <= aff_idx + 4'd1; state <= ST_AFF_START; end
                end
