...
module ed25519_ht_fe17_madd (
    ...
    always_comb begin
        ...
        unique case (state)
            ...
            ST_MUL_CD_START: begin
                mul0_valid = 1'b1;
                mul0_a = p_T;
                mul0_b = q_xy2d;
                mul1_valid = 1'b1;
                mul1_a = p_Z;
                mul1_b = fe17_t'(255'd2);
            end
        ...
        if (!rst_n) begin
            ...
            unique case (state)
                ...
                ST_INIT: begin
                    ...
                    state <= ST_MUL_AB_START;
                end
                ST_MUL_AB_START: state <= ST_MUL_AB_WAIT;
                ST_MUL_AB_WAIT: begin
                    if (mul0_done && mul1_done) begin
                        b_reg <= mul0_y;
                        a_reg <= mul1_y;
                        state <= ST_MUL_CD_START;
                    end
                end
                ST_MUL_CD_START: state <= ST_MUL_CD_WAIT;
                ST_MUL_CD_WAIT: begin
                    if (mul0_done && mul1_done) begin
                        c_reg <= mul0_y;
                        d_reg <= mul1_y;
                        state <= ST_EFGH;
