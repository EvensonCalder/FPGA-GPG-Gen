...
module ed25519_ht_keygen_batch32_pingpong_stage (
    input  logic         clk,
    input  logic         rst_n,
    ...
    logic [255:0] seed_buf [0:1][0:31];
    fe17_t x_buf [0:1][0:31];
    fe17_t y_buf [0:1][0:31];
    fe17_t z_buf [0:1][0:31];
    logic [255:0] batch_seed [0:31];
    logic [255:0] batch_public [0:31];
    logic batch_start;
    logic batch_done;
    ...
    ed25519_ht_fe17_batch_affine32 u_batch (
            ...
            unique case (proc_state)
                ...
                PROC_DONE: begin
                    if (out_valid && out_ready) begin
                        if (drain_idx == 5'd31) begin
                            proc_bank <= !proc_bank;
                            proc_state <= PROC_IDLE;
                        end else begin
                            drain_idx <= drain_idx + 5'd1;
                        end
                    end
                end
