...
module gpg_vanity_pattern_matcher #(
    ...
    wire suffix_match =
        ...
        (suffix[15:12] == suffix[11:8])  &&
        (suffix[15:12] == suffix[7:4])   &&
        (suffix[15:12] == suffix[3:0]);

    wire prefix_match =
        (prefix[31:28] == prefix[27:24]) &&
        (prefix[31:28] == prefix[23:20]) &&
        (prefix[31:28] == prefix[19:16]) &&
        (prefix[15:12] == prefix[11:8])  &&
        (prefix[15:12] == prefix[7:4])   &&
        (prefix[15:12] == prefix[3:0]);

    always @* begin
        if (DEBUG_ACCEPT_ALL) begin
            hit = 1'b1;
            class_id = 4'h0;
        end else if (MATCH_SUFFIX && suffix_match) begin
            hit = 1'b1;
            class_id = 4'h0;
        end else if (MATCH_PREFIX && prefix_match) begin
            hit = 1'b1;
            class_id = 4'h1;
        end else begin
            hit = 1'b0;
